var BASIS = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    function findWasmBinary() {
      return locateFile('basis_encoder_threads.wasm')
    }
          ...
          return signed
            ...
            : function (pointer) {
                return this.fromWireType(
                  (growMemViews(), HEAPU16)[pointer >> 1],
                )
              }
        case 4:
          return signed
            ...
            : function (pointer) {
                return this.fromWireType(
                  (growMemViews(), HEAPU32)[pointer >> 2],
                )
              }
        default:
          throw new TypeError(`invalid integer width (${width}): ${name}`)
    ...
    var UTF16ToString = (ptr, maxBytesToRead, ignoreNul) => {
      var idx = ptr >> 1
      var endIdx = findStringEnd(
        (growMemViews(), HEAPU16),
        idx,
        maxBytesToRead / 2,
        ignoreNul,
      )
    ...
    var emval_returnValue = (toReturnWire, destructorsRef, handle) => {
      ...
      if (destructors.length) {
        ;(growMemViews(), HEAPU32)[destructorsRef >> 2] =
    ...
    function __munmap_js(addr, len, prot, flags, fd, offset) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(11, 0, 1, addr, len, prot, flags, fd, offset)
      offset = bigintToI53Checked(offset)
      try {
        var stream = SYSCALLS.getStreamFromFD(fd)
        if (prot & 2) {
    ...
    var __tzset_js = (timezone, daylight, std_name, dst_name) => {
      ...
      ;(growMemViews(), HEAP32)[daylight >> 2] = Number(
    ...
    function _environ_get(__environ, environ_buf) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(12, 0, 1, __environ, environ_buf)
      var bufSize = 0
      var envp = 0
    ...
    function _fd_read(fd, iov, iovcnt, pnum) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(15, 0, 1, fd, iov, iovcnt, pnum)
      try {
        var stream = SYSCALLS.getStreamFromFD(fd)
    ...
    function _fd_seek(fd, offset, whence, newOffset) {
      if (ENVIRONMENT_IS_PTHREAD)
        return proxyToMainThread(16, 0, 1, fd, offset, whence, newOffset)
      offset = bigintToI53Checked(offset)
      try {
    ...
    function assignWasmExports(wasmExports) {
      ...
      _free = wasmExports['qa']
      _pthread_self = wasmExports['sa']
      _malloc = wasmExports['ta']
      __emscripten_tls_init = wasmExports['ua']
      _emscripten_builtin_memalign = wasmExports['va']
      __emscripten_thread_init = wasmExports['wa']
      __emscripten_thread_crashed = wasmExports['xa']
      __emscripten_run_js_on_main_thread = wasmExports['ya']
      __emscripten_thread_free_data = wasmExports['za']
      __emscripten_thread_exit = wasmExports['Aa']
      __emscripten_check_mailbox = wasmExports['Ba']
      _setThrew = wasmExports['Ca']
    ...
    function assignWasmImports() {
      wasmImports = {
        ...
        X: __emscripten_init_main_thread_js,
        P: __emscripten_notify_mailbox_postmessage,
        S: __emscripten_receive_on_main_thread_js,
        B: __emscripten_thread_cleanup,
        W: __emscripten_thread_mailbox_await,
        ba: __emscripten_thread_set_strongref,
        ...
        ma: _emscripten_has_threading_support,
